// Proposal instruction parser — browser port of agent/src/decode.ts.
// Turns raw (targets, values, calldatas) into structured "intents".
import { decodeFunctionData, slice, type Address, type Hex } from "viem";
import { treasuryAbi } from "./abi";

export type Intent =
  | {
      kind: "treasury-withdraw";
      target: Address;
      asset: Address;
      to: Address;
      amount: bigint;
...
function decodeAction(target: Address, calldata: Hex, value: bigint): Intent {
  // A Treasury.withdraw call is recognized by its ABI (selector 0xd9caed12).
  try {
...
/// Does this intent move funds out of the given treasury (target == Treasury)?
export function isTreasuryDrain(intent: Intent, treasury: Address): boolean {
  return (
    intent.kind === "treasury-withdraw" &&
    intent.target.toLowerCase() === treasury.toLowerCase()
  );
}
